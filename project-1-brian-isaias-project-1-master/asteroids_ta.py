        ...
        def expand(self, outer):
            ...
            for action in ['e','d','c','s']:
                ...
                if self.parent and self.parent.move[0] == action:
                    continue 
                if action == 's':
                    # at 1 fuel, choosing s means you will never be able to
                    # finish, as you would need more fuel to start moving again 
                    if self.state.ship.fuel == 1:
                        continue
                    for time in range(10, 201, 10):
                        state = outer.act(self.state,action,time)
                        child = outer.Node(self,state,(action,time))
                        self.leaves[(action,time)] = child
                else:
                    for time in range(10, 101, 5):
                        state = outer.act(self.state,action,time)
                        child = outer.Node(self,state,(action,time))
                        self.leaves[(action,time)] = child
...
def main(args):
    ...
    return(bfs.state.ship.fuel - len(path), path)
